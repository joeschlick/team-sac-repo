    ...
    for (let i = 2; i < apiFunction.data.length; i++) {
      ...
      if (i === 2) {
        $("#five-day-forecast")
          .append(`<div class="card small-6 large-2 columns" id="weather-card">
            <h5 class="fdf" style="padding-top: 10px;">Today In: ${
              apiFunction.city_name
            } </h5>
            <img class="images" id="image-hover-effect" src="https://www.weatherbit.io/static/img/icons/${
              apiFunction.data[i].weather.icon
            }.png">
            <p class="fdf">High Temp: ${Math.round(highTempInFahrenheit)}</p>
            <p class="fdf">Low Temp: ${Math.round(lowTempInFahrenheit)}</p>
            <p class="fdf">Current Condition: ${
              apiFunction.data[i].weather.description
            }
            <p class="fdf">Wind-Direction: ${apiFunction.data[i].wind_cdir_full}
            <p class="fdf" style="padding-bottom: 4px;">Gusts up to: ${Math.round(
              kphConvertGusts
            )} MPH
            </div>`);
      } else if (i >= 7) {
      ...
      $("#five-day-forecast")
        .append(`<div class="card small-6 large-2 columns" id="weather-card">
             ...
             <img class="images tooltiptext" id="image-hover-effect" src="https://www.weatherbit.io/static/img/icons/${
               apiFunction.data[i].weather.icon
             }.png">
            <p class="fdf">High Temp: ${Math.round(highTempInFahrenheit)}</p>
            ...
            <p class="fdf">Wind-Direction: ${apiFunction.data[i].wind_cdir_full}
            <p class="fdf" style="padding-bottom: 10px;">Gusts up to: ${Math.round(
              kphConvertGusts
            )} MPH
            </div>`);
    }
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (articleAbstract) {
        articleContent
          ...
          .attr("class", "article-row-content-description");
      }
